...
export const latenessService = {
  ...
  deleteLateness: (latenessIds: number[]) => {
    ...
    if (result.changes === 0) {
      throw createError({
        statusCode: 404,
        message: "لم يتم حذف التأخرات المحددة",
      });
    }
    if (result.changes < latenessIds.length) {
      throw createError({
        statusCode: 409,
        message:
          "تم حذف بعض التأخيرات المحددة، لكن بعضها لم يتم العثور عليه (تم حذف " +
          result.changes +
          " من " +
          latenessIds.length +
          ")",
  ...
  createLateness: (lateness: NewLateness[]) => {
    ...
    if (result.changes < lateness.length) {
      throw createError({
        statusCode: 409,
        message:
          "تم إضافة بعض التأخيرات الجديدة، لكن بعضها لم يتم إضافته (تم إضافة " +
          result.changes +
          " من " +
          lateness.length +
          ")",
  ...
  batchEditLateness: (lateness: BatchEditLateness) => {
    ...
    if (result.changes < lateness.ids.length) {
      throw createError({
        statusCode: 207,
        message:
          "تم تحديث بعض التأخيرات المحددة، لكن بعضها لم يتم العثور عليه (تم تحديث " +
          result.changes +
          " من " +
          lateness.ids.length +
          ")",
