<template>
    ...
    <div class="card flex justify-center">
        ...
        <Stepper value="1" linear class="w-4/5 sm:w-[40rem]">
            <StepList>
                <Step value="1" v-show="fromOtherClassesCandidates.length">نقل من أقسام أخرى</Step>
                <Step value="2" v-show="seasonTerminationActive">حذف</Step>
                <Step value="3" v-show="seasonTerminationActive && studentPromotionActive">التأكيد</Step>
            </StepList>
            <StepPanels>
                <!-- Step 1 : add season data -->
                <StepPanel v-slot="{ activateCallback }" value="1" v-if="fromOtherClassesCandidates.length">
                    <div class="flex flex-col gap-4">
                        <p>هؤلاء الطلبة سيتم نقلهم إلى القسم الذي حددته</p>
...
import type { NewSchoolSeason, NewSeasonPayload, ClassPromotionMap, Student, XLSXStudent } from '~/data/types';
/*A prop used to suggest current season termination to the user */
const props = defineProps<{
    fromOtherClassesCandidates: XLSXStudent[],
    removingCandidates: Student[]
}>();
...
const nextStepClick = async (step: number, formActivateCallback: (value: string | number) => void) => {
    if (step === 1) {
        ...
        if (studentPromotionActive.value) {
